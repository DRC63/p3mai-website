...
def footer(doc, classification, version):
    ...
    r = p.add_run("P3MAI Website"); _set_font(r, BODY_FONT, 8, GREY)
...
def title_page(doc, doc_id, title, subtitle, version, date, author, classification):
    ...
    p = doc.add_paragraph(); p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    r = p.add_run("BUSINESS WEBSITE"); _set_font(r, HEAD_FONT, 14, GOLD_DARK, bold=True)
    r.font.spacing = Pt(2)
    doc.add_paragraph()
    _rule(doc, NAVY)
    p = doc.add_paragraph(); p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    ...
    meta = [("Document", f"{doc_id} — {title}"), ("Version", version),
            ...
            ("Classification", classification), ("Applies to", "P3MAI Website (p3mai.com)")]
