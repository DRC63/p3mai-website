...
| contact.html | Validated form, Google Map (DIFC Dubai), hours. **Form REALLY sends email since 2026-08-06**: posts to `contact.php` (PHP mail(), From noreply@p3mai.com for SPF, Reply-To = visitor, honeypot field, header-injection guard) → drcolvin@p3mai.com. Verified end-to-end live (test submission received in the inbox). Prefills Message from ?interest=/?details= params (see script.js). Failure path shows a direct-email fallback. |
| calculator.html | Original simple web-project cost estimator (self-contained, real-time totals) |
| **p3mai_services_cost_estimator.html** | **The primary estimator** — real P3MAI service catalogue. Three towers, twelve fixed-price products in three families, POTI multiplier for consulting, product tier and licence multipliers, costed maintenance window, sticky summary rail, AED/GBP/USD. Linked from services.html ("Get an Instant Cost Estimate"), with a "← Back to Services" return link. Its "Get a Detailed Quote" button carries the full itemized basket to contact.html via ?details=. **Pricing logic: `memory/approach/pricing-model.md`.** |
| **4 article pages** (2026-08-06, MKT-10 done) | Full Read-More case studies behind the blog teasers: `building-web-apps-with-claude.html` (flagship — full-transparency Claude build story, incident log included), `ai-programs-need-a-pmo.html`, `scaling-ai-infrastructure.html`, `agentic-ai-in-the-pmo.html`. First-person, ~1,000 words, public-facts-only confidentiality rail, takeaways box + context-carrying CTA each. Article CSS: `.article-meta/.article-body/.takeaways` in style.css. |
